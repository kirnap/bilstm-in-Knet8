# Single lstm for substitute generation
...
function test(param, state, data; perp=false)
    ...
    for sequence in data
        val = loss(param, copy(state), sequence) #val = loss(param, state, sequence)
...
function main(args=ARGS)
    ...
    param = initparams(o[:atype], o[:layerconfig], o[:embedding], vocabsize, o[:winit])
